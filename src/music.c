...
#define qcansee(x,y) (!quiet && (cansee((x),(y))))

/* Generate earthquake :-) of desired force.
 * That is:  create random chasms (pits).
 */
void
really_do_earthquake(force, ox, oy, big, quiet)
int force, ox, oy;
boolean big;
boolean quiet;
{
            ...
            if (!rn2(14 - force))
                ...
                do_pit:
                    ...
                    if (mtmp) {
                            ...
                            if (!m_already_trapped) { /* suppress messages */
                                if (qcansee(x, y))
                                    pline("%s falls into a chasm!",
                                          Monnam(mtmp));
                                else if (humanoid(mtmp->data) && !quiet)
                                    You_hear("a scream!");
                            }
                        ...
                        if (u.utrap && u.utraptype == TT_BURIEDBALL && !quiet) {
                        ...
                        if (Levitation || Flying
                            ...
                            if ((!tu_pit) && (!quiet)) { /* no pit here previously */
...
void do_earthquake(force, ox, oy)
int force, ox, oy;
{
    really_do_earthquake(force, ox, oy, FALSE, FALSE);
}

void do_big_earthquake(force, quiet)
int force;
boolean quiet;
{
    really_do_earthquake(force, 0, 0, TRUE, quiet);
}
...
#define PLAY_HALLU    0x04
    ...
    case PLAY_CONFUSED:
        ...
        break;
    case PLAY_HALLU:
        You("produce a kaleidoscopic display of floating butterflies.");
        break;
