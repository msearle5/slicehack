...
/* return TRUE if the corpse has special timing;
   lizards and lichen don't rot, trolls and Riders etc. auto-revive
   **/
#define special_corpse(num) \
    (((num) == PM_LIZARD) || ((num) == PM_LICHEN) || ((num) == PM_LEGENDARY_LICHEN) \
     || ((num) == PM_SLOTH) \
     || (is_rider(&mons[num])) || (is_reviver(&mons[num])))
...
struct obj *add_to_magic_chest(struct obj *obj,int key)
{
    ...
    if (obj->where != OBJ_FREE)
		panic("add_to_magic_chest: obj not free (%d,%d,%d)", obj->where, obj->otyp, obj->invlet);

    /* merge if possible */
    for (otmp = magic_chest_objs[key]; otmp; otmp = otmp->nobj)
		if (merged(&otmp, &obj))
			return otmp;
