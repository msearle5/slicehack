...
#define m_initsgrp(mtmp, x, y) m_initgrp(mtmp, x, y, 0, 3)
#define m_initlgrp(mtmp, x, y) m_initgrp(mtmp, x, y, 0, 10)
#define m_inithgrp(mtmp, x, y) m_initgrp(mtmp, x, y, 6, 12)
...
STATIC_OVL void
m_initgrp(mtmp, x, y, b, n)
register struct monst *mtmp;
register int x, y, b, n;
{
    coord mm;
...
register struct monst *mtmp;
{
    ...
    case S_LEMMING:
        switch (mm) {
            ...
            if (rn2(2))
                mongets(mtmp, PIL_HALLUCINATION);
            break;
             ...
             * Sometimes they also get a mirror or a lantern. */
            {
                ...
                if (rn2(2))
                    mongets(mtmp, PIL_HALLUCINATION);
                if (rn2(3))
                    mongets(mtmp, PIL_SPEED);
                if (rn2(4))
                    mongets(mtmp, PIL_CONFUSION);
                if (rn2(2))
            ...
            if (rn2(3)) mongets(mtmp, WAN_FREEZE_RAY);
            ...
            if (!rn2(3)) mongets(mtmp, SCR_FIRE);
            if (rn2(2)) mongets(mtmp, WAN_FIRE_BLAST);
...
register struct monst *mtmp;
{
    ...
    case S_HUMAN:
        if (is_mercenary(ptr)) {
            ...
            if (!rn2((ptr == &mons[PM_ALIGNED_PRIEST]) ? 4 : 20))
                mongets(mtmp, HOLY_SYMBOL);
    ...
    if (anymon) {
        ...
        else
        {
            if ((ptr->geno & (G_SGROUP|G_LGROUP)) == (G_SGROUP|G_LGROUP)) {
                if (rn2(3))
                    m_inithgrp(mtmp, mtmp->mx, mtmp->my);
                else
                    m_initlgrp(mtmp, mtmp->mx, mtmp->my);
            } else {
