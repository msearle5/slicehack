...
static const char * const hallu_adverb[] = {
    ...
    "not very", "not really", "kinda", "vaguely", "strangely", "oddly", "totally",
    "utterly", "properly", "'orribly", "thoroughly", "stridently", "fervently", "nominally",
    "unexpectedly", "considerably", "horridly", "majorly", "righteously", "scarily"
};
...
struct monst *mon;
{
    ...
    if (mon->mhp == mon->mhpmax || mon->mhp < 1)
        return (char *)0;
    if (!Hallucination && mon->mhp <= mon->mhpmax / 7) {
        Sprintf(buf,"almost ");
        strcat(buf, nonliving(mon->data) ? "destroyed" : "dead");
    } else {
        if (Hallucination) {
            Strcpy(buf,hallu_adverb[rn2(SIZE(hallu_adverb))]);
            strcat(buf," ");
        }
        else if (mon->mhp <= mon->mhpmax / 4)
            Sprintf(buf,"horribly ");
        else if (mon->mhp <= 2 * mon->mhpmax / 5)
            Sprintf(buf,"severely ");
...
maybe_wail()
{
    ...
    } else if (Subrole_if(PM_WIZARD) || Race_if(PM_ELF) || Role_if(PM_VALKYRIE)) {
        const char *who;
        int i, powercnt;

        who = (Subrole_if(PM_WIZARD) || Role_if(PM_VALKYRIE)) ? urole.name.m
                                                           : "Elf";
        if (u.uhp == 1) {
