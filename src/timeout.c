...
boolean yours;
{
    ...
    (void) stop_timer(BOMB_BLOW, obj_to_any(bomb));
    bomb->yours = yours;
    bomb->oarmed = TRUE;
...
long timeout;
{
    ...
    if (get_obj_location(bomb, &x, &y, BURIED_TOO | CONTAINED_TOO)) {
        switch(bomb->where) {
            ...
            pline("%s explodes in your %s!", someth, where);
            losehp(d(2 * bomb->quan,5), "carrying live explosives", KILLED_BY);
            break;
    ...
    if (timeout != monstermoves) {
        ...
        if (how_long >= obj->age) {
            ...
            if (menorah) {
                ...
                bomb_blow(obj_to_any(obj), timeout);
                return;
            }
        } else {
            obj->age -= how_long;
