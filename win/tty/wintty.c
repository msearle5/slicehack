...
STATIC_DCL void FDECL(bail, (const char *)); /* __attribute__((noreturn)) */
STATIC_DCL void FDECL(setup_subrolemenu, (winid, BOOLEAN_P, int, int, int, int));
STATIC_DCL void FDECL(setup_rolemenu, (winid, BOOLEAN_P, int, int, int, int));
STATIC_DCL void FDECL(setup_racemenu, (winid, BOOLEAN_P, int, int, int, int));
STATIC_DCL void FDECL(setup_gendmenu, (winid, BOOLEAN_P, int, int, int, int));
STATIC_DCL void FDECL(setup_algnmenu, (winid, BOOLEAN_P, int, int, int, int));
STATIC_DCL boolean NDECL(reset_role_filtering);
STATIC_DCL boolean NDECL(reset_subrole_filtering);
#ifdef STATUS_HILITES
...
#define SUBROLE flags.initsubrole
...
tty_player_selection()
{
     ...
     */
    picksomething = (SUBROLE ==ROLE_NONE || ROLE == ROLE_NONE || RACE == ROLE_NONE
    ...
    if (SUBROLE == ROLE_NONE || ROLE == ROLE_NONE || RACE == ROLE_NONE || GEND == ROLE_NONE
        || ALGN == ROLE_NONE) {
        int echoline;
        char *prompt = build_plselection_prompt(pbuf, QBUFSZ,
                                                SUBROLE, ROLE, RACE, GEND, ALGN);
 ...
 makepicks:
    ...
    do {
        if (nextpick == RS_ROLE) {
            ...
            if (ROLE < 0) {
                ...
                if (pick4u == 'y' || pick4u == 'a' || ROLE == ROLE_RANDOM) {
                    /* Pick a random role */
                    k = pick_role(SUBROLE, RACE, GEND, ALGN, PICK_RANDOM);
                    if (k < 0) {
                    ...
                    setup_rolemenu(win, TRUE, SUBROLE, RACE, GEND, ALGN);
		...
		if (nextpick == RS_SUBROLE) {
            nextpick = RS_RACE;
            /* Select a subrole, if necessary;
               we'll try to be compatible with pre-selected
               role/race/gender/alignment, but may not succeed. */
            if ((SUBROLE < 0) && (has_subroles(ROLE, RACE, GEND, ALGN))) {
                /* Process the choice */
                if (pick4u == 'y' || pick4u == 'a' || SUBROLE == ROLE_RANDOM) {
                    /* Pick a random subrole */
                    k = pick_subrole(ROLE, RACE, GEND, ALGN, PICK_RANDOM);
                    if (k < 0) {
                        tty_putstr(BASE_WINDOW, 0, "Incompatible subrole!");
                        k = randsubrole();
                    }
                } else {
                    /* Prompt for a subrole */
                    tty_clear_nhwindow(BASE_WINDOW);
                    role_selection_prolog(RS_SUBROLE, BASE_WINDOW);
                    win = create_nhwindow(NHW_MENU);
                    start_menu(win);
                    /* populate the menu with subrole choices */
                    setup_subrolemenu(win, TRUE, ROLE, RACE, GEND, ALGN);
                    /* add miscellaneous menu entries */
                    role_menu_extra(ROLE_RANDOM, win, TRUE);
                    any = zeroany; /* separator, not a choice */
                    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE, "",
                    ...
                    if (gotsubrolefilter())
                        role_menu_extra(RS_filter, win, FALSE);
                    role_menu_extra(ROLE_NONE, win, FALSE); /* quit */
                    Strcpy(pbuf, "Pick a subrole or specialization");
                    end_menu(win, pbuf);
                    n = select_menu(win, PICK_ONE, &selected);
                    /*
                     * PICK_ONE with preselected choice behaves strangely:
                    ...
                    if (choice == ROLE_NONE) {
                        ...
                        ROLE = k = ROLE_NONE;
                        (void) reset_subrole_filtering();
                        nextpick = RS_ROLE;
                    } else if (choice == ROLE_RANDOM) {
                        k = pick_subrole(SUBROLE, RACE, GEND, ALGN, PICK_RANDOM);
                        if (k < 0)
                            k = randsubrole();
                    } else {
                        k = choice - 1;
            ...
            if (RACE < 0 || !validrace(SUBROLE, ROLE, RACE)) {
                ...
                if (pick4u == 'y' || pick4u == 'a' || RACE == ROLE_RANDOM) {
                    ...
                    for (i = 0; races[i].noun; i++)
                        if (ok_race(SUBROLE, ROLE, i, GEND, ALGN)) {
                            n++;
                            k = i;
                        }
                    if (n == 0) {
                        for (i = 0; races[i].noun; i++)
                            if (validrace(SUBROLE, ROLE, i)) {
                                n++;
                                k = i;
                            }
                    }
                    ...
                    if (n > 1) {
                        ...
                        setup_racemenu(win, TRUE, SUBROLE, ROLE, GEND, ALGN);
                        /* add miscellaneous menu entries */
                        role_menu_extra(ROLE_RANDOM, win, TRUE);
            ...
            if (GEND < 0 || !validgend(SUBROLE, ROLE, RACE, GEND)) {
                ...
                if (pick4u == 'y' || pick4u == 'a' || GEND == ROLE_RANDOM) {
                    ...
                    for (i = 0; i < ROLE_GENDERS; i++)
                        if (ok_gend(SUBROLE, ROLE, RACE, i, ALGN)) {
                            n++;
                            k = i;
                        }
                    if (n == 0) {
                        for (i = 0; i < ROLE_GENDERS; i++)
                            if (validgend(SUBROLE, ROLE, RACE, i)) {
                                n++;
                                k = i;
                            }
                    }
                    ...
                    if (n > 1) {
                        ...
                        setup_gendmenu(win, TRUE, SUBROLE, ROLE, RACE, ALGN);
                        /* add miscellaneous menu entries */
                        role_menu_extra(ROLE_RANDOM, win, TRUE);
                        any.a_int = 0; /* separator, not a choice */
                        add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE, "",
            ...
            if (ALGN < 0 || !validalign(SUBROLE, ROLE, RACE, ALGN)) {
                ...
                if (pick4u == 'y' || pick4u == 'a' || ALGN == ROLE_RANDOM) {
                    ...
                    for (i = 0; i < ROLE_ALIGNS; i++)
                        if (ok_align(SUBROLE, ROLE, RACE, GEND, i)) {
                            n++;
                            k = i;
                        }
                    if (n == 0) {
                        for (i = 0; i < ROLE_ALIGNS; i++)
                            if (validalign(SUBROLE, ROLE, RACE, i)) {
                                n++;
                                k = i;
                            }
                    }
                    ...
                    if (n > 1) {
                        ...
                        start_menu(win);
                        any = zeroany; /* zero out all bits */
                        setup_algnmenu(win, TRUE, SUBROLE, ROLE, RACE, GEND);
                        role_menu_extra(ROLE_RANDOM, win, TRUE);
                        any.a_int = 0; /* separator, not a choice */
                        add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE, "",
                                 MENU_UNSELECTED);
                        ...
                        if (choice == ROLE_NONE) {
                            ...
                            ROLE = k = ROLE_NONE;
                            nextpick = RS_ROLE;
                        } else if (choice == RS_menu_arg(RS_SUBROLE)) {
                            ROLE = k = ROLE_NONE;
                            nextpick = RS_SUBROLE;
                        } else if (choice == RS_menu_arg(RS_filter)) {
                            ALGN = k = ROLE_NONE;
    ...
    } while ((has_subroles(ROLE, RACE, GEND, ALGN) && (SUBROLE < 0)) || ROLE < 0 || RACE < 0 || GEND < 0 || ALGN < 0);
    ...
    while (getconfirmation) {
        ...
        case 3: { /* 'a' */
             ...
             */
            int saveSUBROLE, saveROLE, saveRACE, saveGEND, saveALGN;

            iflags.renameinprogress = TRUE;
            /* plnamesuffix() can change any or all of ROLE, RACE,
               GEND, ALGN; we'll override that and honor only the name */
            saveSUBROLE = SUBROLE, saveROLE = ROLE, saveRACE = RACE, saveGEND = GEND,
                saveALGN = ALGN;
            *plname = '\0';
            plnamesuffix(); /* calls askname() when plname[] is empty */
            ROLE = saveSUBROLE, ROLE = saveROLE, RACE = saveRACE, GEND = saveGEND,
                ALGN = saveALGN;
        ...
        case 2:    /* 'n' */
            ...
            pick4u = 'n';
            SUBROLE = ROLE = RACE = GEND = ALGN = ROLE_NONE;
            goto makepicks;
            break;
...
reset_subrole_filtering()
{
    ...
    setup_rolemenu(win, FALSE, ROLE_NONE, ROLE_NONE, ROLE_NONE, ROLE_NONE);

    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE, "", MENU_UNSELECTED);
	add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE,
             "Unacceptable subroles", MENU_UNSELECTED);
    setup_rolemenu(win, FALSE, ROLE_NONE, ROLE_NONE, ROLE_NONE, ROLE_NONE);

    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE, "", MENU_UNSELECTED);
    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE,
             "Unacceptable races", MENU_UNSELECTED);
    setup_racemenu(win, FALSE, ROLE_NONE, ROLE_NONE, ROLE_NONE, ROLE_NONE);

    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE, "", MENU_UNSELECTED);
    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE,
             "Unacceptable genders", MENU_UNSELECTED);
    setup_gendmenu(win, FALSE, ROLE_NONE, ROLE_NONE, ROLE_NONE, ROLE_NONE);

    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE, "", MENU_UNSELECTED);
    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE,
             "Unacceptable alignments", MENU_UNSELECTED);
    setup_algnmenu(win, FALSE, ROLE_NONE, ROLE_NONE, ROLE_NONE, ROLE_NONE);
    ...
    if (n > 0) {
        clearsubrolefilter();
        for (i = 0; i < n; i++)
            setsubrolefilter(selected[i].item.a_string);
...
STATIC_OVL void
setup_rolemenu(win, filtering, subrole, race, gend, algn)
winid win;
boolean filtering; /* True => exclude filtered roles; False => filter reset */
int subrole, race, gend, algn; /* all ROLE_NONE for !filtering case */
{
    anything any;
    int i;
    boolean role_ok;
    ...
    for (i = 0; roles[i].name.m; i++) {
        role_ok = ok_role(subrole, i, race, gend, algn);
...
/* add entries to menu being built in 'win' */
STATIC_OVL void
setup_subrolemenu(win, filtering, role, race, gend, algn)
winid win;
boolean filtering;
int role, race, gend, algn;
{
    ...
    for (i = 0; subroles[i].name.m; i++) {
        subrole_ok = ok_subrole(i, role, race, gend, algn);
        if (filtering && !subrole_ok)
            continue;
        if (filtering)
            any.a_int = i + 1;
        else
            any.a_string = subroles[i].name.m;
        this_ch = *subroles[i].name.m;
        /* filtering: picking subrole, so choose by first letter, with
        ...
        add_menu(win, NO_GLYPH, &any,
                 ...
                 ATR_NONE, subroles[i].name.m,
                 (!filtering && !subrole_ok) ? MENU_SELECTED : MENU_UNSELECTED);
    }
}

STATIC_OVL void
setup_racemenu(win, filtering, subrole, role, gend, algn)
winid win;
boolean filtering;
int subrole, role, gend, algn;
{
    anything any;
    boolean race_ok;
    int i;
    ...
    any = zeroany;
    for (i = 0; races[i].noun; i++) {
        race_ok = ok_race(subrole, role, i, gend, algn);
...
STATIC_DCL void
setup_gendmenu(win, filtering, subrole, role, race, algn)
winid win;
boolean filtering;
int subrole, role, race, algn;
{
    anything any;
    boolean gend_ok;
    int i;
    ...
    any = zeroany;
    for (i = 0; i < ROLE_GENDERS; i++) {
        gend_ok = ok_gend(subrole, role, race, i, algn);
...
STATIC_DCL void
setup_algnmenu(win, filtering, subrole, role, race, gend)
winid win;
boolean filtering;
int subrole, role, race, gend;
{
    anything any;
    boolean algn_ok;
    int i;
    ...
    any = zeroany;
    for (i = 0; i < ROLE_ALIGNS; i++) {
        algn_ok = ok_align(subrole, role, race, gend, i);
...
static const enum statusfields
    ...
    { BL_TITLE, BL_STR, BL_DX, BL_CO, BL_IN, BL_WI, BL_CH, BL_ALIGN,
      BL_SCORE, BL_FLUSH, blPAD, blPAD, BL_WEIGHT, BL_WEIGHTMAX, blPAD },
    ...
    { BL_TITLE, BL_STR, BL_DX, BL_CO, BL_IN, BL_WI, BL_CH,
      BL_SCORE, BL_FLUSH, blPAD, blPAD, blPAD, BL_WEIGHT, BL_WEIGHTMAX, blPAD },
